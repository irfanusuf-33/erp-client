...
export default function IamReviewGroup({ setView, formData }: IamStepProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.data?.success) { setError(res.data?.msg || "Failed to create group"); showError("Failed to Create Group", res.data?.msg || "Failed to create group"); setLoading(false); return; }
      showSuccess("Group Created", "Group has been created successfully."); router.push("/iam/groups");
